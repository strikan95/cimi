...
abstract class BaseFixture extends Fixture
{
    ...
    protected function createManyAlternative(int $count, callable $factory, mixed $data = null): void
    {
        for ($i = 0; $i < $count; $i++) {
            $entity = $factory($i, $data);
            $this->manager->persist($entity);
            // store for usage later as App\Entity\ClassName_#COUNT#
            $this->addReference(get_class($entity) . '_' . $i + $this->createdCount, $entity);
        }

        $this->createdCount += $count;
    }

    protected function createOne(string $className, callable $factory, mixed $data = null): void
    {
    ...
    protected function getRandomNReferences(string $className, int $min, int $max): Collection
    {
        $this->loadReferenceIndex($className);
    ...
    private function loadReferenceIndex(string $className): void
    {
        ...
        if (empty($this->referencesIndex[$className])) {
            die(sprintf('Cannot find any references for class "%s"', $className));
        }
    }
}