...
class ListingFixture extends BaseFixture implements DependentFixtureInterface
{
    ...
    protected function loadData(ObjectManager $manager): void
    {
        $this->createManyAlternative(200, function () {
            $propertyListing = new Listing($this->getRandomReference(User::class));
            $propertyListing->setTitle($this->faker->realText(32));
            $propertyListing->setDescription($this->faker->realText(46));
            $propertyListing->setStructureType($this->getRandomReference(StructureType::class));
            $propertyListing->setPlaceType($this->getRandomReference(PlaceType::class));
            $propertyListing->setPrice(rand(300, 2000));
            $propertyListing->setAmenities($this->getRandomNReferences(Amenity::class, 6, 26));
            $propertyListing->setLocation(
    ...
    private function generateRandomImage(): Image
    {
        ...
        $image->setThumbnailUrl('https://picsum.photos/id/'.$randId.'/720/480');
    ...
    {
        return [$rect[0] + $this->randomFloat(0, $rect[2] - $rect[0]), $rect[1] + $this->randomFloat(0, $rect[3] - $rect[1])];
    }
